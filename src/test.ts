import { ComplexPoint } from './shapes';
import { GPU } from "gpu.js";
const gpu = new GPU()

const test = function(x: number, y: number ,maxX: number, maxY: number) {
    ...
    do{
        ...
        zRealTemp += a
        zImagineTemp += b
        if(Math.sqrt(zRealTemp**2 + zImagineTemp**2) > 2){
            trigered = true
            break;
        }
        i++
...
const normal =  function(x: number, y: number ,maxX: number, maxY: number){
    const a =  (x - maxX/2)/(maxX/4)
    const b =  (y - maxY/2)/(maxY/4)     
    const c = new ComplexPoint(a,b)
    let z = new ComplexPoint(0,0)
    let trigered = false
    let i = 0
    do{
        z = z.power(2)
